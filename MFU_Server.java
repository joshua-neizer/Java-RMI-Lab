...
// Frame Sever that follows the Most Frequently Used page replacement policy
class MFU_Server extends java.rmi.server.UnicastRemoteObject implements Central_Interface {
    
    // Variables used in the MFU server
    private static int frame_count = 5;
    private int[] frames;
    private int[] FIFO_queue;
    private HashMap<Integer, Integer> MFU_log;
    ...
    /**
    * MFU Server constructor; instantiates variables and connects to the central
    * server.
    */ 
    public MFU_Server() throws RemoteException {
        ...
        this.MFU_log = new HashMap<Integer, Integer>();

        // Tries to connect to the Central Server
        System.out.println("MFU_Server> Starting up ...");
        try {
    ...
    * current request based on the MFU model
    *
    * @param  request   the integer client request
    */ 
    private void frameReplacement(int request) throws RemoteException {
        ...
        if (this.occupied_frames < frame_count){
            ...
            int[] victim = new int[] {0, this.MFU_log.get(this.frames[0])};
            int current_frame;

            // Iterates over the frames to replace the victim frame
            for (int i = 1; i < frame_count; i++){
                // Gets current frame information
                current_frame = this.MFU_log.get(this.frames[i]);

                // Replaces victim frame if a larger frame count is found
            ...
            this.MFU_log.put(this.frames[victim [0]], 0);
    ...
    */ 
    public int request(String client_name, int request) throws RemoteException{
        ...
        System.out.printf("MFU_Server> Request from %s for integer %d...\n", client_name, request);

        this.request_count ++;
        
        // Checks for a page fault and handles it accordingly
        if (this.isPageFault(request)) {
            System.out.printf("MFU_Server> Page Fault detected from %s for integer request %d...\n", client_name, request);
            ...
            S_Server.request("MFU_Server", request);
        }

        // Updates the request count for MFU replacement policy
        if (this.MFU_log.get(request) == null)
            this.MFU_log.put(request, 1);
        else
            this.MFU_log.put(request, this.MFU_log.get(request) + 1);

        System.out.printf("MFU_Server> Frames Illustration %s...\n", this.print_frames());
    ...
    */ 
	public int Compute_Square(int x) throws RemoteException {
		System.out.println("MFU_Server> Computing ...");
 		System.out.println("MFU_Server> Returning "+x*x+" ...");
		return x*x;
    }   
    ...
    */ 
    private void connect_clint(String client) throws RemoteException {
        ...
        this.client_request = Socket_IO.socket_in("MFU_Server", client);
        ...
        Socket_IO.socket_out("MFU_Server", client, server_response);
    }

	/**
    * MFU Server main function
    *
    * @param  args   command line input String array    
    */ 
	public static void main (String args[]) throws RemoteException {
        // Instantiates server
        MFU_Server S_Client = new MFU_Server();
        ...
        System.out.printf("MFU_Server> On 20 references from Client_1, got %d page faults\n", 
                            S_Client.fault_count.get("Client_1"));
        System.out.printf("MFU_Server> On 20 references from Client_2, got %d page faults\n", 
                            S_Client.fault_count.get("Client_2"));
        System.out.printf("MFU_Server> On 40 references from Clients,  got %d page faults\n", 
